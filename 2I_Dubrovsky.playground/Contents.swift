...
// MARK: b. Пусть переменная p изначально равна двум — первому простому числу.
var p = 2

// MARK: e. Повторять шаги c и d, пока возможно.
for i in array {
    
    // MARK: c. Зачеркнуть в списке числа от 2 + p до n, считая шагом p..
    // В описании задания C ошибка, метод Эратосфена фильтрует по 'X * P', а не по 'Х + P'
    for i in array {
        // Проверка. Если 'X * P' больше последнего числа массива, то выхожу из цикла.
        guard (p * i) <= array.last! else {
            break
        }
        // Удаляю элементы массива, соответсвующие 'X * P'
        if let index = array.firstIndex(of: p * i) {
            array.remove(at: index)
        }
    }
    
    // MARK: d. Найти первое не зачёркнутое число в списке, большее, чем p, и присвоить значению переменной p это число.
    if p <= array[i-2] && p < array.last! {
        p = array[i-1]
    } else {
        break
    }
